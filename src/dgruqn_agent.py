...
from src.networks.dGRUqn import DGRUQN
import numpy as np
from tqdm import tqdm

class DGRUQNAgent(BaseAgent):

    def __init__(self, config):
        super(DGRUQNAgent, self).__init__(config)
        self.replay_memory = DRQNReplayMemory(config)
        self.net = DGRUQN(self.env_wrapper.action_space.n, config)
        self.net.build()
    ...
    def policy(self, state):
        ...
        if np.random.rand() < self.epsilon:
            ...
            a, self.gru_state_c, self.gru_state_h = self.net.sess.run([self.net.q_action, self.net.state_output_c, self.net.state_output_h],{
                self.net.state : [[state]],
                self.net.c_state_train: self.gru_state_c,
                self.net.h_state_train: self.gru_state_h
            })
            return a[0]
    ...
    def train(self, steps):
        ...
        t = 0
        self.screen = self.env_wrapper.screen
        self.gru_state_c, self.gru_state_h = self.net.initial_zero_state_single, self.net.initial_zero_state_single

        for self.i in tqdm(range(self.i, steps)):
            # To fix dimension problem
            self.gru_state_c = self.gru_state_c.reshape(1, 512)
            self.gru_state_h = self.gru_state_h.reshape(1, 512) 
            state = self.screen/255
            action = self.policy(state)
            self.env_wrapper.act(action)
            #print("Shape C> ", self.gru_state_c.shape)
            #print("Shape H> ", self.gru_state_h.shape)
            #print("Shape C> ", self.net.c_state_train)
            #print("Shape H> ", self.net.h_state_train)
#
            #print("State> ", state.shape)
            #print("Net state> ", self.net.state.shape)
            if self.random:
                self.gru_state_c, self.gru_state_h = self.net.sess.run([self.net.state_output_c, self.net.state_output_h], {
                    self.net.state: [[state]],
                    self.net.c_state_train : self.gru_state_c,
                    self.net.h_state_train: self.gru_state_h
                })
                # Fix problem of shape
                #self.gru_state_c = self.gru_state_c.reshape(1, self.gru_state_c.shape[0])
                #self.gru_state_h = self.gru_state_h.reshape(1, self.gru_state_h.shape[0]) 
            self.observe(t)
    ...
    def play(self, episodes, net_path):
        ...
        while i < episodes:
            ...
            total_reward += self.env_wrapper.reward
            episode_steps += 1
            #print(self.env_wrapper.reward)
            if episode_steps > self.config.max_steps:
                self.env_wrapper.terminal = True
            if self.env_wrapper.terminal:
                print("Total Reward Episode {} > {}".format(i, total_reward))
                total_reward = 0
                episode_steps = 0
                i += 1
