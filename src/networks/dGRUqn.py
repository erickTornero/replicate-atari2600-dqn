...
class DGRUQN(BaseModel):

    def __init__(self, n_actions, config):
        super(DGRUQN, self).__init__(config, "dgruqn")
    ...
    def add_placeholders(self):
        ...
        self.state_target = tf.placeholder(tf.float32,
                                           ...
                                           name="input_target")
        # create placeholder to fill in gru state
        self.c_state_train = tf.placeholder(tf.float32, [None, self.gru_size], name="train_c")
        self.h_state_train = tf.placeholder(tf.float32, [None, self.gru_size], name="train_h")


        self.c_state_target = tf.placeholder(tf.float32, [None, self.gru_size], name="target_c")
        self.h_state_target = tf.placeholder(tf.float32, [None, self.gru_size], name="target_h")

        # initial zero state to be used when starting episode
        self.initial_zero_state_batch = np.zeros((self.batch_size, self.gru_size))
        self.initial_zero_state_single = np.zeros((1, self.gru_size))

        self.initial_zero_complete = np.zeros((self.num_gru_layers, 2, self.batch_size, self.gru_size))
    ...
    def add_logits_op_train(self):
        ...
        out, state = stateful_gru(out_flat, self.num_gru_layers, self.gru_size,
                                               scope_name="gru_train")
        self.state_output_c = state[0][0]
        # To fix index boud exeption: Warning [0][1]
        self.state_output_h = state[0][0]
        shape = out.get_shape().as_list()
    ...
    def train_on_batch_target(self, states, action, reward, terminal, steps):
        ...
        gru_state_c, gru_state_h = self.initial_zero_state_batch, self.initial_zero_state_batch
        gru_state_target_c, gru_state_target_h = self.sess.run(
        ...
        for i in range(self.min_history):
            # To fix problem of dimenssion
            sh = gru_state_c.shape
            print("shape C> ", sh)
            print("shape H> ",gru_state_h.shape)
            nel = 1
            for ix in range(len(sh)):
                nel *= sh[ix]
            nbatch = 1
            if nel <= 512:
                nbatch = 1
                ...
                nbatch = int(nel/512)
            print("nel", nel)
            print("Nbatch", int(nbatch))
            gru_state_c = gru_state_c.reshape(nbatch, 512)
            gru_state_h = gru_state_h.reshape(nbatch, 512) 
            #print("shape c", gru_state_target_c.shape)
            gru_state_target_c = gru_state_target_c.reshape(1, 512)
            gru_state_target_h = gru_state_target_h.reshape(1, 512)
            # end fixing problem, since it returns the batch in inverse order
            j = i + 1
            gru_state_c, gru_state_h, gru_state_target_c, gru_state_target_h = self.sess.run(
                [self.state_output_c, self.state_output_h, self.state_output_target_c, self.state_output_target_h],
                {
                    self.state: states[i],
                    self.state_target: states[j],
                    self.c_state_target: gru_state_target_c,
                    self.h_state_target: gru_state_target_h,
                    self.c_state_train: gru_state_c,
                    self.h_state_train: gru_state_h
                }
            )
        for i in range(self.min_history, self.min_history + self.states_to_update):
            ...
            gru_state_target_c = gru_state_target_c.reshape(1, 512) 
            gru_state_target_h = gru_state_target_h.reshape(1, 512)
            target_val, gru_state_target_c, gru_state_target_h = self.sess.run(
            ...
            gru_state_c = gru_state_c.reshape(1, 512)
            gru_state_h = gru_state_h.reshape(1, 512) 
            #self.learning_rate = self.learning_rate.reshape(1, 512)
            target = (1. - terminal[i]) * self.gamma * max_target + reward[i]
            _, q_, train_loss_, gru_state_c, gru_state_h, merged_imgs= self.sess.run(
    ...
    def init_update(self):
        ...
        self.gru_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="gru_train")
        gru_target_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="gru_target")

        for i, var in enumerate(self.gru_vars):
            self.target_w_in[var.name] = tf.placeholder(tf.float32, var.get_shape().as_list())
            self.target_w_assign[var.name] = gru_target_vars[i].assign(self.target_w_in[var.name])
