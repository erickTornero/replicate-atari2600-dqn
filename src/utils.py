...
def stateful_lstm(x, num_layers, lstm_size, state_input, scope_name="lstm"):
    with tf.variable_scope(scope_name):
        ...
        print("state> gru >", state)
        return outputs, state

def stateful_gru(x, num_layers, gru_size, scope_name="gru"):
    with tf.variable_scope(scope_name):
        #cell = tf.nn.rnn_cell.LSTMCell(lstm_size, state_is_tuple=True)
        cell = tf.nn.rnn_cell.GRUCell(gru_size)
        cell = tf.nn.rnn_cell.MultiRNNCell([cell]*num_layers)
        #state_input = cell.zero_state(4, dtype=tf.float32)
        state_input = tf.zeros([64, 2])
        #print("X shape> ", x.shape)
        #print("State input> ",state_input)
        #print("Cell> ",cell)
